...
- 10 seed properties, each with 4 images + full editorial descriptions + lat/lng coords.
- Full properties CRUD (admin-gated). PropertyForm supports optional lat/lng.
...
- **Compare Homes** — pin up to 3 properties with a floating bar (persisted in localStorage), side-by-side comparison table with price/type/beds/baths/area/price-per-sqft + amenities matrix.
- **Map View** — Leaflet + CARTO tiles, numbered markers per property, rich popups with thumbnail + price + View listing link. Grid/Map toggle on Listings.
- 23/23 backend tests + full Playwright E2E passing across 3 iterations.
...
- Address → lat/lng geocoding in PropertyForm (instead of manual entry).
